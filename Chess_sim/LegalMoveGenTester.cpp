...
void LegalMoveGenTester::test()
{
    ...
    for (uint32_t i = 0; i < 6; i = i + 1) {
        ...
        if (correct == got) std::cout << "Depth " << std::setw(4) << i << ". Correct: " << std::setw(18) << correct << ". Got: " << std::setw(18) << got << ". Speed: " << std::setw(10) << speed << " MNPS. OK."  << std::endl;
        else std::cout  << "Depth " << std::setw(4) << i << ". Correct: " << std::setw(18) << correct << ". Got: " << std::setw(18) << got << ". Speed: " << std::setw(10) << speed << " MNPS. Error."  << std::endl;
    }
}
uint64_t LegalMoveGenTester::_get_nodes_number(const Position& position, uint8_t side, uint32_t depth) {
    ...
    for (uint8_t i = 0; i < moves.size(); i = i + 1) {
        ...
        copy = position;
        copy.move(move);
        counter = counter + LegalMoveGenTester::_get_nodes_number(copy, Pieces::inverse(side), depth - 1);
    }
